...
def test_extract_qr_token_from_url(settings: Settings) -> None:
    ...
    assert verify_qr_payload(settings, extracted, settings.qr_ttl_seconds) == 7


def test_extract_qr_token_raw(settings: Settings) -> None:
    token = sign_qr_payload(settings, 3)
    assert verify_qr_payload(settings, _extract_qr_token(token), settings.qr_ttl_seconds) == 3
