...
- Кабинет продавца (`/seller`): вход по телефону и паролю, сканирование QR, списание баллов и автоначисление с суммы после списания.
...
Сборка попадает в `app/static/pwa/` (эту папку отдаёт Docker при монтировании `.:/app`).
...
- **продавец** — `POST /auth/seller/login`, проверка QR, проведение покупки (списание + автоначисление), регистрация покупателя;
